...
1. AWS Deploy・GitHub Actions:<br>
GitHubでコードを管理してるため、GitHub ActionsとAWS Deployを併せることで、
...
3. AWS Deployのテストステージを追加<br>
Deployのテストステージを追加することで、テストを実行し忘れてバグが本番環境に混入することを防ぐように設計(要件3対応)

4. ブルーグリーンデプロイ方式<br>
ブルーグリーンデプロイを実装することで、現行の本番環境に影響を与えずすぐに前のバージョンに戻せるため採用。
さらにグリーン環境に切り替えた後もエラーが増えていないか、パフォーマンスが落ちていないかを監視するためにClaud Watchでメトリクス監視を実装(要件5対応)
...
(3) GitHub Actionsでコスト視覚化を図ることで、コストの視認性を高める設計
AWS:<br>
(1) GitHub ActionsからAWSへの認証をOIDC活用<br>
アクセスキーの管理が不要で、セキュリティが向上し運用コストも抑えられる
(2) Lambdaの活用<br>
AWS Deployを使う場合、従量課金制のLambdaを活用することで低コスト化を図った<br>
(要件6対応)
